...
fit_random_forest <- function(train, cfg, ntree = NULL) {
  if (is.null(ntree)) ntree <- max(cfg$models$random_forest$ntree_values)  # 500

  exclude <- c("class_mean", "class_median")
  train_rf <- train %>% dplyr::select(-dplyr::any_of(exclude))

  message(sprintf("[fit_random_forest] Fitting RF (ntree=%d) ...", ntree))
  ...
  model <- randomForest::randomForest(
    ...
    data       = train_rf,
    ntree      = ntree,
    importance = cfg$models$random_forest$importance
  )
